<?php
namespace Cellularisrael\BundleProduct\Block\Catalog\Product\View\Type;
...
class Bundle extends \Magento\Catalog\Block\Product\View\AbstractView
{
    ...
    protected $_advanceBundleHelper;
    ...
    public function __construct(
        ...
        \Cellularisrael\BundleProduct\Helper\Data $advanceBundleHelper,
        array $data = []
    ) {
        ...
        $this->_advanceBundleHelper = $advanceBundleHelper;
        // parent::__construct(
        //     $context,
        //     $arrayUtils,
        //     $catalogProduct,
        ...
        foreach ($optionsArray as $optionItem) {
            ...
            if ($preConfiguredFlag) {
                ...
                if ($configValue) {
                    ...
                    if ($configQty) {                        
                        @$options[$optionId]['selections'][$configValue]['qty'] = (int) $configQty;
                    }
                }
     ...
     * @return array
     */
    private function getSelectionItemData(Product $product, Product $selection, $option)
    {
        ...
        $oldPrice = $product->getPriceInfo()
            ...
            ->getOptionSelectionAmount($selection)
            ->getValue();
        $childProduct = $this->_productFactory->create()->load($selection->getProductId());
        $displayPrice = $finalPrice;
        $currencySymbol = $this->_advanceBundleHelper->getCurrencySymbol();
        if($option->getMerchant() == 2 && $childProduct->getIsraelPrice()){
            $displayPrice = number_format($childProduct->getIsraelPrice(),2);
            $currencySymbol = $this->_advanceBundleHelper->getCurrencySymbol('ILS');
        }
        ...
        $selection = [
            ...
            'canApplyMsrp' => false,
            'isSubscription' => $childProduct->getIsSubscription(),
    ...
    private function getSelections(Option $option, Product $product)
    {
        ...
        foreach ($option->getSelections() as $selectionItem) {
            ...
            $selections[$selectionId] = $this->getSelectionItemData($product, $selectionItem, $option);
    ...
    private function getOptionItemData(Option $option, Product $product, $position)
    {
        return [
            ...
            'position' => $position,
            'merchant' => $option->getMerchant(),
            'sim_custom_addon' => $option->getSimCustomAddon(),
            'phone_custom_addon' => $option->getPhoneCustomAddon()
        ];
    }
